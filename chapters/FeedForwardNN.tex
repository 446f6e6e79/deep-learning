...
\begin{itemize}
    ...
    \end{figure}
    This function is smooth and differentiable, but it saturates (approaches 0) across most of its domain, which can lead to vanishing gradients during training.
    ...
    \end{figure}
    This is better than the sigmoid function as it is zero-centered, which can help with convergence during training, but it still suffers from the saturation problem.
    ...
    However, it can suffer from the "dying ReLU" problem, where neurons can become inactive and only output zero for all inputs, which can hinder learning.
