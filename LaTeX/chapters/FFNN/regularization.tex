...
\end{equation*}
where $\mathcal{L}(\boldsymbol{\theta})$ is the original loss function, $\Omega(\boldsymbol{\theta})$ is the regularization term, and $\lambda$
...
\begin{itemize}
    ...
    \begin{equation*}
        \Omega(\boldsymbol{\theta}) = \frac{1}{2} \sum_{i} ||\theta_i^2||
    \end{equation*}
    ...
    \begin{equation*}
        \Omega(\boldsymbol{\theta}) = \sum_{i} ||\theta_i||
    \end{equation*}
\end{itemize}
\begin{comment}
...
\begin{itemize}
    ...
    This concept is also foundational to \textit{Denoising Autoencoders}, where the network is trained to recover the original input 
    ...
    This encourages the training process to converge to a \textit{flat minimum} of the loss function, 
...
\begin{figure}[H]
    \centering
    \input{LaTeX/Tikz/FFNN/regularization/multi-task-learning.tex}
...
Usually, we have several tasks that share the same set of input data $\boldsymbol{x}$, as well as some intermediate representations $h^{(shared)}$,
...
In fact, it is at the base of \textit{Convolutional Neural Networks} (CNNs), where the same set of weights is shared across different spatial locations in the input image, 
...
\textit{Dropout} is a regularization technique born from the idea of ensemble learning, which allows to approximate the effect of training 
multiple models by randomly dropping out a subset of the neurons during training.
\begin{figure}[H]
    \centering
    \input{LaTeX/Tikz/FFNN/regularization/dropout.tex}
